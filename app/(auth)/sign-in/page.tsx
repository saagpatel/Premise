...
export default function SignInPage() {
	...
	const handleMagicLink = async (e: React.FormEvent) => {
		...
		// Lazy init — must not call at module/render time (no env vars during SSR pre-render)
	...
	const handleGitHub = async () => {
		...
		// If there's an error, OAuth redirect didn't happen
		if (authError) {
			setIsLoading(false);
			setError(authError.message);
		}
			...
			<div className="w-full max-w-md">
				<div className="mb-8 text-center">
					...
					<p className="mt-3 text-base font-light text-gray-500">
						Save your debates and set a username.
					</p>
				</div>
						...
						<>
							...
							<form onSubmit={handleMagicLink}>
								...
								<button
									...
									className={clsx(
										"mt-4 w-full rounded-lg px-6 py-3 text-sm font-bold transition-colors",
							...
							<button
								...
								disabled={isLoading}
								className={clsx(
									"flex w-full items-center justify-center gap-3 rounded-lg border px-6 py-3 text-sm font-bold transition-colors",
